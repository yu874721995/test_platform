...
from api_case.models import SwaggerApi, Case
from api_case.serializers import SwaggerSerializer
from test_management.models import moduleMent
...
class SwaggerViewSet(ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        # 使用线程预处理有用例的接口
        def have_case():
            query_2 = SwaggerApi.objects.filter(~Q(status='有用例'))
            swagger_id = []
            for obj in query_2:
                case = Case.objects.filter(only_api=obj.only_api)
                if case:
                    swagger_id.append(obj.id)
            SwaggerApi.objects.filter(id__in=swagger_id).update(status="有用例")
...
def sync_swagger(request):
    ...
    # url_list_all = [
    #                 "http://10.0.1.149/nextop-finance/full.api.json",
    #                 "http://10.0.1.149/nextop-finance-receivable/full.api.json",
    #                 ]  # 调试使用
