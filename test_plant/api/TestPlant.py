...
class TestPlantView():
    ...
    def create(cls, request):
        ...
        dingding = json_request(request, 'dingding')
        ...
        account = manageModels.ErpAccount.objects.filter(id=account_id).values()[0]['account']
        cookie = manageModels.ErpAccount.objects.filter(id=account_id).values()[0]['cookie']
        ...
        with transaction.atomic():
            ...
            try:
                if task_type == 'date':
                    ...
                    scheduler.add_job(test_plant, id=task_id, run_date=datetimes, args=[task_name,id,task_id,creator,case_list,group_list,project_id,env[1],account_id,account,cookie,creator])
                elif task_type == 'now':
                    scheduler.add_job(test_plant, id=task_id, run_date=datetime.datetime.now(), args=[task_name,id,task_id,creator,case_list,group_list,project_id,env[1],account_id,account,cookie,creator])
                else:
                    add_job(test_plant, task_id, task_type, times, [task_name,id,task_id,creator,case_list,group_list,project_id,env[1],account_id,account,cookie,creator], start_time, end_time)
                if status == 1:
    ...
    def update(cls, request):
        ...
        dingding = json_request(request, 'dingding',int,not_null=False,default=None)
        project_id = json_request(request, 'project_id')
        env = json_request(request, 'env',list)
        account_id = json_request(request, 'account_id',int)
        account = manageModels.ErpAccount.objects.filter(id=account_id).values()[0]['account']
                ...
                try:
                    if status == 1:
                        if task_type == 'date':
                            ...
                            datetimes = timezone.datetime.strptime(datetimes,"%Y-%m-%dT%H:%M:%S.%fZ") + datetime.timedelta(hours=8)#前端传的UTC时间。。。
                            modify_job(test_plant, task_id, task_type, datetimes, [task_name,plant_id,task_id,creator,case_list,group_list,project_id,env[1],account_id,account,cookie,creator])
                        else:
                            modify_job(test_plant, task_id, task_type, times, [task_name,plant_id,task_id,creator,case_list,group_list,project_id,env[1],account_id,account,cookie,creator], start_time,
                                       end_time)
                        scheduler.resume_job(task_id)
                    elif status == 2:
                        scheduler.pause_job(task_id)
    ...
    def plant_executions_list(cls, request):
        ...
        nex_query = []
        for query in querys:
            env_name = manageModels.envList.objects.filter(id=int(query['env'])).values()[0]['lable_name']
            query['env'] = env_name
