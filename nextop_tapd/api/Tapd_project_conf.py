...
class tapd_project_conf():

    @classmethod
    @request_verify('post', {'tapd_project_id': int,'owner_change_push':str},
    ...
    def create(cls, request):
        ...
        start_time = json_request(request, 'start_time', not_null=False, default=None)  # 不填则不限制
        end_time = json_request(request, 'end_time', not_null=False, default=None)  # 不填则不限制
        webhook_url = json_request(request, 'webhook_url')
        owner_change_push = json_request(request, 'owner_change_push',str)
        ...
        try:
            tapd_models.tapd_project_conf.objects.create(
                **{
                    ...
                    'users': users,
                    'creator': creator,
                    'owner_change_push':owner_change_push
                }
            )
        except Exception as e:
            logger.error('添加tapd推送配置失败：{}'.format(str(e)))
            return HttpResponse(json.dumps({'code': 10005, 'msg': '添加失败'}))
    ...
    def page(cls, request):
        ...
        querys = tapd_models.tapd_project_conf.objects.filter(query).values().order_by('-create_time')
        datas = []
        if querys:
            for query in querys:
                if query['users']:
    ...
    @classmethod
    @request_verify('post', {'id': int, 'tapd_project_id': int,'owner_change_push':str},
                    {'demand_before_status': list, 'demand_after_status': list, 'users': list})
